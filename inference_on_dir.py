...
if __name__ == '__main__':
    ...
    parser.add_argument('--thresholds', type=str, nargs='*')
    parser.add_argument('--num-workers', type=int, default=6)
    parser.add_argument('--batch-size', type=int, default=32)
    parser.add_argument('--fold', type=int, default=0)
    parser.add_argument('--cfg', type=str, default='base_config.yaml')
    args = parser.parse_args()
    print(args)
    import sys
    src_path = abspath(join(args.experiment_dir, args.exp, f'fold_{args.fold}/saved_src'))

    cfg = yaml.safe_load(open(join(src_path, f'config/{args.cfg}')))
    cfg['general']['weights_path'] = None
    scored_birds = cfg['general']['scored_birds']

    scores = [float(x) for x in args.thresholds]
    thresholds = {k:v for k, v in zip(bird_names, scores)}
    ...
    with open(join(args.experiment_dir, args.exp, f'fold_{args.fold}/weights/best_model.txt')) as file:
        best_model_name = file.readline()
    weights_path = torch.load(join(args.experiment_dir, args.exp, f'fold_{args.fold}/weights', best_model_name))['model']
            ...
            for batch_idx, (filename, score) in enumerate(zip(data['filename'],  prediction)):
                ...
                for offset_index, offset in enumerate(range(0, 60, 5)):
                    for bird, index in zip(bird_names, scored_birds):
                        ...
                        pred['target'].append(True if score[offset_index][index] > thresholds[bird] else False)
