...
class Trainer():
    ...
    def train_single_fold(self, train_folds, val_folds):
        cfg = self.cfg
        train_ds = CustomDataset(cfg, train=True, folds=train_folds)
        ...
        val_ds = CustomDataset(cfg, train=False, folds=val_folds)
        ...
        for metric in self.metric_list:
            ...
            if isinstance(submetrics, dict):
                ...
                for name, value in submetrics.items():
                    self.logger.log_value(f'{name}/train', value, epoch)
                    self.logger(f'{name}/train - {value:.4f}')
    ...
    def validate(self, val_loader, epoch):
        ...
        for metric in self.metric_list:
            ...
            if isinstance(submetrics, dict):
                submetrics = metric.compute()
                for name, value in submetrics.items():
                    self.logger.log_value(f'{name}/val', value, epoch)
                    self.logger(f'{name}/val - {value:.4f}')
                    self.metric_dict[name] = value    
            else:
                value = submetrics
