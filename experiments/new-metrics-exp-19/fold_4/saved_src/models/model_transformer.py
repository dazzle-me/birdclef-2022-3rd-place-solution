...
class Net(nn.Module):
    ...
    def forward(self, data):
        ...
        # print(x.shape)
        x = torch.nn.functional.interpolate(x, size=(self.cfg['data']['spec_width'], self.cfg['data']['spec_height']))
        # print(x.shape)
        
        bs, channels, time, mels = x.shape
        empty_bank = torch.zeros(bs, channels, 224, 224, dtype=x.dtype).to(self.cfg['general']['device'])
        
        # print(x.shape)
        ## x.shape (bs, channels, 196, 256)
        x1 = x.view(bs, channels, 14, 14, 16, 16)
        for i in range(14):
            for j in range(14):
                empty_bank[:, :, i*16:(i+1)*16, j*16:(j+1)*16] = x1[:, :, i, j]

        # print(empty_bank.shape)
        x = self.backbone.forward_features(empty_bank)
        # print(x.shape)
        if self.is_train:
            bs, time, channels = x.shape
            x = x.reshape(b // self.factor, self.factor * time, channels)
        x = x.mean(dim=1)
        # print(x.shape)
        logits = self.head(x)
