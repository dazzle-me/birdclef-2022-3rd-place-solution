...
from models.layers.cutmix import Cutmix

from models.layers.concat_mix import ConcatMix
from models.layers.random_power import RandomPower
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        params = cfg['training']['augs']['spec_augment']
        self.spec_augment = SpecAugmentation(
            time_drop_width=params['time_drop_width'],
            time_stripes_num=params['time_stripes_num'],
            freq_drop_width=params['freq_drop_width'],
            freq_stripes_num=params['freq_stripes_num'],
        )
        ...
        self.random_power = RandomPower(cfg)
        ...
        self.cutmix = Cutmix(mix_beta=cfg['training']['augs']['mix_beta'])
    ...
    def forward(self, data):
        ...
        if self.is_train:
            ...
            if np.random.random() <= self.cfg['training']['augs']['concat_mix']:
                ...
                current_factor *= 2
            bs, time = x.shape
            x = x.reshape(bs * current_factor, time // current_factor)
        else:
            x = data['audio']
            y = data['target']
            weight = data['weight']
        ...
        if self.is_train:
            ...
            x = x.reshape(b // current_factor, current_factor * t, c, f) ## (bs, time, 1, mel)
            if np.random.random() <= self.cfg['training']['augs']['mixup']:
                x, y, weight = self.mixup(x, y, weight)
            if np.random.random() <= self.cfg['training']['augs']['cutmix']:
            ...
            x = x.reshape(b, t, c, f)
            x = x.permute(0, 2, 1, 3)
            if np.random.random() <= self.cfg['training']['augs']['spec_augment']['proba']:
                x = x.permute(0, 1, 3, 2) ## (bs, 1, mel, time)
                x = self.spec_augment(x)
                x = x.permute(0, 1, 3, 2) ## (bs, 1, time, mel)
        ...
        if self.is_train:
            b, c, t, f = x.shape
            x = x.permute(0, 2, 1, 3)
            x = x.reshape(b // current_factor, current_factor * t, c, f)
            x = x.permute(0, 2, 1, 3)
